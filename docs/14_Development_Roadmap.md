...
**Status:** Done — `crates/contexa-vision` (window monitor, WGC capture, UIA extraction, frame differencing + region hashing, exclusion filter, adaptive scheduler, `VisionEngine`/`ContexaVisionEngine` with the ADR-0008 Pattern A capture thread). Selective OCR is now implemented (`Windows.Media.Ocr` against a cropped region, own one-shot COM thread) after `spikes/SP-03-ocr-fallback` (Pass, 2026-07-16 — see its `report.md` and `benchmarks/BASELINE.md`) validated latency, accuracy, and CPU usage all within target. `ExclusionFilter::default_rules()` seeds password-manager/financial/healthcare app exclusions (docs/16 §6.2) at the composition root (`apps/desktop`); UIA extraction redacts password fields (`element.is_password()` → `[REDACTED]`, docs/16 §7.1). 27 unit tests pass (pure logic only); WinAPI/COM/GPU paths including live OCR verified via `examples/vision_smoke.rs` against a real window.
...
**Status:** Done — `crates/contexa-context` (`SnapshotAssembler`, `ContextCache`, `ChangeDetector`, `PluginRegistry`/`PluginSandbox`, Chrome/Edge enricher via targeted UIA address-bar lookup, VS Code enricher via window-title parsing — see its module doc for why, not live UIA — Language Detector via `whatlang`, Selection Tracker via UIA `TextPattern` + clipboard fallback, and `ContexaContextEngine` wiring it all together with a `tokio::sync::broadcast` `subscribe()`). Selection tracking defaults to a no-op (`NoSelectionSource`) and is opt-in via `enable_selection_tracking()` — exercising real UIA/clipboard state from `cargo test`'s many parallel threads (no message pump) crashed with `STATUS_HEAP_CORRUPTION` during development, so it's enabled only by the real composition root (`apps/desktop`), never in tests. 41 unit tests pass; live UIA-dependent paths (Chrome/Edge/VS Code enrichment, UIA/clipboard selection) verified via `examples/context_smoke.rs` and `examples/vision_smoke.rs` against real windows — both correctly captured the actual foreground window and clipboard content in a live run.
...
**Status:** Done — `crates/contexa-orchestrator` (`DecisionEngine`, `PipelineManager`, `ContexaOrchestrator`); provider selection + fallback lives in `contexa-llm::ProviderSelector` (tries primary, falls back to secondary on failure) rather than in this crate. Response streaming uses a `ResponseStream`/`take_stream` oneshot-channel handback (documented deviation from the spec's direct-return signature — see `engine.rs` module doc). `pipeline.rs` now triggers real OCR via `Region::whole_window()` (clamped to the actual frame by `ocr.rs::crop()`) whenever `plan.need_ocr` is set, closing the OCR-fallback path end-to-end (R-T01). Known stand-in pending a real signal: `uia_confidence`/`should_search` in `decision.rs` are heuristic placeholders (no confidence score surfaced from Vision yet, no search-intent classifier) — accepted for v1. 10 unit tests pass; `examples/orchestrator_smoke.rs` exercises the live pipeline.
